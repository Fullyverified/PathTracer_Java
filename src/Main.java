import java.util.ArrayList;
import java.util.List;

public class Main {

    public static int RenderResolutionX = 500;
    public static int aspectX = 4;
    public static int aspectY = 3;
    public static int fov = 60;
    public static int frameTime = 50; // Milliseconds
    public static int raysPerPixel = 2000;
    public static int bouncesPerRay = 20;
    public static boolean ASCIIMode = false;

    public static void main(String[] args) {

        List<SceneObjects> sceneObjectsList = new ArrayList<>();

        sceneObjectsList.add(new AABCubeCenter(0, -3, 0, 1,1,1,1,1,1,1));
        //sceneObjectsList.add(new Sphere(0,-3,0,1,1,1,1,0.75));

        sceneObjectsList.add(new SphereLight(0, 2,0, 1,40,40,40,1,1,1,1));

        Camera cam = new Camera(1, RenderResolutionX, fov, aspectX, aspectY, 0,0,0, 0, -1, 0, 0, 1, 0);

        cam.directionVector();
        cam.upVector();
        cam.rightVector();
        cam.imagePlane();

        RenderSingleThreaded renderSingleThreaded = new RenderSingleThreaded();
        renderSingleThreaded.computePixels(sceneObjectsList, cam, raysPerPixel, bouncesPerRay, frameTime, ASCIIMode);
    }
}